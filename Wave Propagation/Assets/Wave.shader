	...
	SubShader {
		...
		Pass{
			...
			fixed4 frag(v2f_img i) : SV_Target{
				static const float3 e = float3(float2(1., 1.) / _ScreenParams.xy, 0.);
				float2 q = i.uv;
		...
		Pass {
			...
			static const float2 simRainDrop = float2(0.456665, 0.708618);
			...
			fixed4 frag(v2f_img i) : SV_Target{
				...
				else
				{
					...
					float t = _Time.y*2.;
					//float2 pos = frac(floor(t)*float2(0.456665,0.708618))*_ScreenParams.xy;
					float2 pos = frac(floor(t)*simRainDrop)*_ScreenParams.xy;
					float amp = 1. - step(.05,frac(t));
					d = -amp*smoothstep(2.5,.5,length(pos - (i.uv.xy *_ScreenParams.xy)));
				}
		...
		Pass{
			...
			fixed4 frag(v2f_img i) : SV_Target{
				...
				float4 c = tex2D(iChannel1, ((i.uv.xy *_ScreenParams.xy)*1.) / (_ScreenParams.xy + grad.xy*.35));//1);
